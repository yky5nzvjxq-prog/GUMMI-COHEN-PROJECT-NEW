...
export default function Drawings() {
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-navy-600 mb-6">שרטוטים שהועלו</h1>
      ...
      {orders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {orders.map(order => (
            <Link key={order.id} to={`/orders/${order.id}`} className="bg-white rounded-xl shadow-sm p-5 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3 mb-3">
                <div className="bg-blue-50 p-2.5 rounded-lg">
                  <FileImage size={20} className="text-blue-500" />
                </div>
                <div>
                  <div className="font-medium text-sm">{getDrawingName(order)}</div>
